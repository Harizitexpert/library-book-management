...
const initialMembers = [
  { id:1, name:'Aminah Ali', email:'aminah@gmail.com', borrowed:0 },
  { id:2, name:'John Lee', email:'john10@gmail.com', borrowed:0 },
  { id:3, name:'Siti Nur', email:'siti@yahoo.com', borrowed:2 }
];
...
// Render category table and chart
function renderCategoryTable(){
  ...
  // Update chart after table render
  renderCategoryChart();
}
...
function renderCategoryChart() {
  ...
  const bgColor = isDark ? 'rgba(111, 66, 193, 0.6)' : 'rgba(75, 192, 192, 0.6)';
  const borderColor = isDark ? 'rgba(111, 66, 193, 1)' : 'rgba(75, 192, 192, 1)';
  const fontColor = isDark ? '#eaeaea' : '#3e2c76';
  ...
  categoryChartInstance = new Chart(ctx, {
      ...
      options: {
          ...
          scales: {
              ...
              y: { beginAtZero: true, ticks: { stepSize: 1, color: fontColor } }
          }
      }
  });
}
...
toggleBtn.addEventListener('click', () => {
    ...
    renderCategoryChart(); // update chart colors
});
